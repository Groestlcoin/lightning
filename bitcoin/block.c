...
static void bitcoin_block_pull_dynafed_params(const u8 **cursor, size_t *len, sph_groestl512_context *shactx)
{
	...
	groestl512_update(shactx, &type, 1);
	...
	case DYNAFED_PARAMS_COMPACT:
		...
		groestl512_varint(shactx, l1);
		groestl512_update(shactx, *cursor, l1);
		pull(cursor, len, NULL, l1);

		/* signblock_witness_limit */
		groestl512_update(shactx, *cursor, 4);
		pull(cursor, len, NULL, 4);

		/* Skip elided_root */
		groestl512_update(shactx, *cursor, 32);
		       ...
		       const char *hex, size_t hexlen)
{
	...
	if (is_elements(chainparams)) {
		...
		if (is_dynafed) {
			...
			groestl512_varint(&shactx, templen);
			groestl512_update(&shactx, p, templen);
	...
	groestl512_double_done256(&shactx, &b->hdr.hash.shad);

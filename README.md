...
Core Lightning only works on Linux and macOS, and requires a locally (or remotely) running `groestlcoind` (version 25.0 or above) that is fully caught up with the network you're running on, and relays transactions (ie with `blocksonly=0`).
...
* [newaddr](https://docs.corelightning.org/reference/newaddr): get a groestlcoin address to deposit funds into your lightning node.

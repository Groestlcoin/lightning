...
static bool smells_like_old_testnet(const char *config_basedir)
{
	...
	/* Does it have a groestlcoin/ subdir and no testnet/ subdir? */
	if (stat(path_join(base, config_basedir, "groestlcoin"), &st) == 0
			 ...
			 char **rpc_filename)
{
	...
	*config_basedir = NULL;
	...
	opt_register_early_noarg("--mainnet",
				 opt_set_specific_network, "groestlcoin",
	...
	if (!chainparams) {
		/* Use groestlcoin default on new installations. */

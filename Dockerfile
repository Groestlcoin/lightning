...
ARG GROESTLCOIN_VERSION=27.0
ARG GROESTLCOIN_URL=https://github.com/Groestlcoin/groestlcoin/releases/download/v${GROESTLCOIN_VERSION}
ARG GROESTLCOIN_TARBALL=groestlcoin-${GROESTLCOIN_VERSION}-${target_arch}.tar.gz

WORKDIR /opt/groestlcoin

ADD ${GROESTLCOIN_URL}/${GROESTLCOIN_TARBALL}    .
ADD ${GROESTLCOIN_URL}/SHA256SUMS            .
ADD ${GROESTLCOIN_URL}/SHA256SUMS.asc        .
COPY contrib/keys/groestlcoin/               gpg/

RUN gpg --quiet --import gpg/* && \
    gpg --verify SHA256SUMS.asc SHA256SUMS && \
    sha256sum -c SHA256SUMS --ignore-missing

RUN tar xzf ${GROESTLCOIN_TARBALL} --strip-components=1
...
COPY --from=downloader    /opt/groestlcoin/bin/groestlcoin-cli	/usr/bin/
